...
func (w *localFileWatcher) run(onNewRecord func(parser.Header, []byte, []byte)) {
	for line := range w.t.Lines {
		lineContent := []byte(line.Text)
		h, p, err := parser.ParseHeaderWithCustomTimeFormat(lineContent, w.format)
