...
func buildTimeIntervalCondition(tx *sql.Tx, clock timeutil.Clock) (query string, args []interface{}, err error) {
	defer func() {
		...
		if sErr := meta.Store(context.Background(), dbconn.Db("intel"), []meta.Item{{Key: alreadyExecutedFlag, Value: true}}); sErr != nil {
			err = sErr
		}
	}()
	...
	err = meta.Retrieve(context.Background(), dbconn.Db("intel"), alreadyExecutedFlag, &alreadyExecuted)
