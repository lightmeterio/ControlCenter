	...
	Convey("Test Auth", t, func() {
		...
		Convey("Register User Fails", func() {
			Convey("Empty password", func() {
				err := auth.Register(dummyContext, "user@example.com", "Name Surname", "")
				So(errors.Is(err, ErrWeakPassword), ShouldBeTrue)
			})

			Convey("Password is equal to email", func() {
				err := auth.Register(dummyContext, "user@example.com", "Name Surname", "user@example.com")
				So(errors.Is(err, ErrWeakPassword), ShouldBeTrue)
			})

			Convey("Password is equal to name", func() {
				err := auth.Register(dummyContext, "user@example.com", strongPassword, strongPassword)
				So(errors.Is(err, ErrWeakPassword), ShouldBeTrue)
			})

			Convey("Invalid email", func() {
				err := auth.Register(dummyContext, "not@an@email.com", "Name Surname", strongPassword)
				So(errors.Is(err, ErrInvalidEmail), ShouldBeTrue)
			})

			Convey("Dictionary password", func() {
				err := auth.Register(dummyContext, "user@email.com", "Name Surname", "ElvisForever")
				So(errors.Is(err, ErrWeakPassword), ShouldBeTrue)
			})

			Convey("Empty Name", func() {
				err := auth.Register(dummyContext, "user@email.com", "   ", strongPassword)
			...
			Convey("Multiple users is forbidden", func() {
				...
				err := auth.Register(dummyContext, "user@email.com", "Valid Name", strongPassword)
				So(err, ShouldBeNil)

				err = auth.Register(dummyContext, "another.user@email.com", "Another User", strongPassword)
		...
		Convey("Register Multiple Users", func() {
			...
			So(auth.Register(dummyContext, "user.one@example.com", "User One", user1Passwd), ShouldBeNil)
			So(auth.Register(dummyContext, "user.two@example.com", "User Two", user2Passwd), ShouldBeNil)
			...
			Convey("Double registration of the same user fails", func() {
				...
				err = auth.Register(dummyContext, "user@email.com", "Another Valid User", `67567567HGFHGFHGhgfghfhg***&*`)
				So(errors.Is(err, ErrUserAlreadyRegistred), ShouldBeTrue)
			})
		})

		Convey("Register User", func() {
			err := auth.Register(dummyContext, "user@example.com", "Name Surname", strongPassword)
			So(err, ShouldBeNil)
			ok, err := auth.HasAnyUser(dummyContext)
			So(err, ShouldBeNil)
			So(ok, ShouldBeTrue)
			...
			Convey("Registering the same user again fails", func() {
				err := auth.Register(dummyContext, "user@example.com", "Another Surname", strongPassword)
...
func TestResetPassword(t *testing.T) {
	Convey("Reset Password", t, func() {
...

		{
			...
			So(err, ShouldBeNil)
			defer func() { So(auth.Close(), ShouldBeNil) }()
			So(auth.Register(dummyContext, "email@example.com", `Nora`, `(1Yow@byU]>`), ShouldBeNil)
		}
