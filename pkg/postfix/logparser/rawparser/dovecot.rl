...
%%{
  ...
  dbType = ('sql' | 'passwd-file' | 'ldap' | 'pam' | 'lua' | 'bsdauth' | 'shadow') >setTokBeg %{
    r.DB = data[tokBeg:p]
  };
  ...
  unknownUserMessage = 'unknown user' any+ %{
    r.DovecotAuthFailedReasonUnknownUser = data[tokBeg:p]
  };

  # TODO: implement other messages, interpolated with '|'
  message = (passwdMismatchMessage | unknownUserMessage);
