...
func TestLightmeterMilters(t *testing.T) {
	Convey("Parse logs from Lightmeter milters", t, func() {
		...
		Convey("Parse References header created by our milter with many values", func() {
			_, parsed, err := Parse(`Jan 20 19:48:04 teupos lightmeter/headers[161]: B9996EABB6: header name="References", value="<abc@example.com> <yxz@example.com>"`)
			So(err, ShouldBeNil)
			So(parsed, ShouldNotBeNil)
			...
			So(p.Values, ShouldResemble, []string{`abc@example.com`, `yxz@example.com`})
			So(p.Value, ShouldEqual, "<abc@example.com> <yxz@example.com>")
		})

		// FIXME: this test fails when running with `-race` flag and I have literally no idea why!!!
		// The issue happens as `some_arbitrary_data` is found, it's not pushed to the `Values` field in the payload
		// I suspect some issue with the the code generated by ragel when running with the race detector instrumentation!!!
		//Convey("Parse references header with some non-message-ids on it, usually arbitrary data", func() {
