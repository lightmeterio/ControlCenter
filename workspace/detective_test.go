	...
	Convey("Detective on real logs", t, func() {
		...
		Convey("File with a successful delivery", func() {
			...
			Convey("Message found", func() {
				...
				So(messages, ShouldResemble, &detective.MessagesPage{1, 1, 1, 1,
					[]detective.MessageDelivery{detective.MessageDelivery{expectedTime.In(time.UTC), "sent", "2.0.0"}},
				})
			})
			...
			Convey("Page number too big", func() {
				...
				So(err, ShouldBeNil)
				So(messages, ShouldResemble, &detective.MessagesPage{2, 1, 1, 0, []detective.MessageDelivery{}})
			})
			...
			Convey("Message out of interval", func() {
				...
				So(messages, ShouldResemble, &detective.MessagesPage{1, 1, 1, 0, []detective.MessageDelivery{}})
			})
		})
		...
		Convey("File with an expired message", func() {
			...
			Convey("Message found", func() {
				...
				So(messages, ShouldResemble, &detective.MessagesPage{1, 1, 1, 5,
