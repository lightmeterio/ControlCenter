...
func TryToEscalateRequest(ctx context.Context, d detective.Detective, requester Requester, from, to string, interval timeutil.TimeInterval, queueName string) error {
	...
	messages, err := d.CheckMessageDelivery(ctx, from, to, interval, -1, queueName, page)
	if err != nil {
