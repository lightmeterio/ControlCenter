...
func buildPublisherAndTempTrackerWithNodeTypeHandler(t *testing.T, handler NodeTypeHandler) (*fakeResultPublisher, *Tracker, func()) {
	pub := &fakeResultPublisher{}

	conn, closeConn := testutil.TempDBConnectionMigrated(t, "logtracker")

	tracker, err := New(conn, pub, handler)
	So(err, ShouldBeNil)
...
func buildMultiNodePublisherAndTempTracker(t *testing.T) (*fakeResultPublisher, *Tracker, func()) {
	return buildPublisherAndTempTrackerWithNodeTypeHandler(t, &MultiNodeTypeHandler{})
}

func buildSingleNodePublisherAndTempTracker(t *testing.T) (*fakeResultPublisher, *Tracker, func()) {
	return buildPublisherAndTempTrackerWithNodeTypeHandler(t, &SingleNodeTypeHandler{})
}

func TestMostRecentLogTime(t *testing.T) {
	Convey("Obtain most recent time", t, func() {
		_, t, clear := buildMultiNodePublisherAndTempTracker(t)
		defer clear()
...
func TestTrackingFromUnsupportedLogFiles(t *testing.T) {
	...
	Convey("Some strange and for now unsupported log lines, that need to be supported in the future!", t, func() {
		pub, t, clear := buildMultiNodePublisherAndTempTracker(t)
		defer clear()
		done, cancel := runner.Run(t)
...
func TestSingleNodeTrackingFromFiles(t *testing.T) {
	Convey("Tracking Single Node Logs From Files", t, func() {
		pub, t, clear := buildSingleNodePublisherAndTempTracker(t)
		_ = pub
		defer clear()
		...
		// FIXME: this test is broken with the multi-server support
		// That happens because the initial connection, which trigger the e-mail message, is not authenticated
		// and sent from localhost. We don't have enough information to judge whether it's been generated by another system
		// and just relayed. So it's never processed!!!
		// TODO: change the criteria of first connection to something independent from authentication.
		// as there are use cases where a message is trriggered via something else than authenticated SMTP (local scripts, for instance)
		...
		// FIXME: from now on, the code is copied from TestMultiNodeTrackingFromFiles!!!
		// It should be refactored in a way that promotes reuse!!!
		...
		Convey("With Tracker", func() {
			Convey("Well behaving files", func() {
				...
				Convey("After being deferred many times, postfix just gives up and set the message as expired", func() {
					...
					So(pub.results[0][ResultStatusKey].Int64(), ShouldEqual, parser.DeferredStatus)
					So(pub.results[0][ResultDeliveryTimeKey].Int64(), ShouldEqual, timeutil.MustParseTime(`2020-09-25 18:26:36 +0000`).Unix())

					So(pub.results[1][ResultStatusKey].Int64(), ShouldEqual, parser.DeferredStatus)
					So(pub.results[1][ResultDeliveryTimeKey].Int64(), ShouldEqual, timeutil.MustParseTime(`2020-09-25 19:01:05 +0000`).Unix())

					So(pub.results[2][ResultStatusKey].Int64(), ShouldEqual, parser.DeferredStatus)
					So(pub.results[2][ResultDeliveryTimeKey].Int64(), ShouldEqual, timeutil.MustParseTime(`2020-09-30 12:46:06 +0000`).Unix())

					So(pub.results[3][ResultStatusKey].Int64(), ShouldEqual, parser.DeferredStatus)
					So(pub.results[3][ResultDeliveryTimeKey].Int64(), ShouldEqual, timeutil.MustParseTime(`2020-09-30 16:46:07 +0000`).Unix())
					...
					So(pub.results[5][MessageExpiredTime].Int64(), ShouldEqual, timeutil.MustParseTime(`2020-09-30 20:46:08 +0000`).Unix())
					...
					So(pub.results[6][ResultDeliveryTimeKey].Int64(), ShouldEqual, timeutil.MustParseTime(`2020-09-30 20:46:08 +0000`).Unix())
...
func TestMultiNodeTrackingFromFiles(t *testing.T) {
